...
const mockStreakData: StreakData[] = [
    {
        ...
        milestones: [
            ...
            { days: 30, title: "Mes Completo", reward: "Sesión gratis", achieved: false },
            { days: 50, title: "Imparable", reward: "Suplemento gratis", achieved: false },
    ...
    },
    {
        ...
        milestones: [
            ...
            { days: 21, title: "Hábito Nutricional", reward: "Consulta nutricional", achieved: false },
            { days: 30, title: "Mes Saludable", reward: "Plan de comidas", achieved: false },
        ],
    },
    {
        studentId: "3",
        ...
        milestones: [
            ...
            { days: 50, title: "Seguimiento Experto", reward: "Plan personalizado", achieved: false },
        ],
    },
];
...
export function StreakTrackingSystem() {
    ...
    const filteredStreaks = streaks.filter((streak) => {
        const matchesSearch = streak.studentName.toLowerCase().includes(searchTerm.toLowerCase())
        const matchesType = typeFilter === "all" || streak.streakType === typeFilter
        const matchesStatus = statusFilter === "all" || streak.streakStatus === statusFilter

        return matchesSearch && matchesType && matchesStatus
    })

    const handleStreakAction = (studentId: string, action: "freeze" | "extend") => {
        setStreaks(
            streaks.map((streak) =>
                streak.studentId === studentId
                    ? {
                        ...streak,
                        streakStatus: action === "freeze" ? "active" : streak.streakStatus,
    ...
    const stats = {
        ...
        avgStreak: Math.round(streaks.reduce((sum, s) => sum + s.currentStreak, 0) / streaks.length),
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold tracking-tight">Sistema de Rachas</h2>
                    <p className="text-muted-foreground">Monitorea y motiva la consistencia de tus estudiantes</p>
                </div>
            </div>

            {/* Stats Cards */}
            <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                <StatCard icon={Flame} color="text-orange-600" label="Total Rachas" value={stats.totalStreaks} />
                <StatCard icon={Flame} color="text-green-600" label="Activas" value={stats.activeStreaks} />
                <StatCard icon={AlertTriangle} color="text-yellow-600" label="En Riesgo" value={stats.atRiskStreaks} />
                <StatCard icon={Flame} color="text-gray-400" label="Rotas" value={stats.brokenStreaks} />
                <StatCard icon={TrendingUp} color="text-blue-600" label="Promedio" value={stats.avgStreak} />
                <StatCard icon={Trophy} color="text-yellow-600" label="Récord" value={stats.longestStreak} />
            </div>
                ...
                <CardContent>
                    <Tabs defaultValue="all">
                        <TabsList className="grid w-full grid-cols-5">
                            <TabsTrigger value="all">Todas ({filteredStreaks.length})</TabsTrigger>
                            <TabsTrigger value="workout">Entrenamiento ({streaksByType.workout.length})</TabsTrigger>
                            <TabsTrigger value="nutrition">Nutrición ({streaksByType.nutrition.length})</TabsTrigger>
                            <TabsTrigger value="weigh_in">Pesaje ({streaksByType.weigh_in.length})</TabsTrigger>
                            <TabsTrigger value="check_in">Check-in ({streaksByType.check_in.length})</TabsTrigger>
                        </TabsList>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                {filteredStreaks.map((streak) => (
                                    <StreakCounter
                                        ...
                                        streakData={streak}
                                        showRiskAlert={true}
                                        onStreakAction={(action) => handleStreakAction(streak.studentId, action)}
                                    />
                                ))}
                            </div>
                        </TabsContent>
                            ...
                            <TabsContent key={type} value={type} className="mt-6">
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                    {typeStreaks
                                        .filter(
                                            (streak) =>
                                                streak.studentName.toLowerCase().includes(searchTerm.toLowerCase()) &&
                                ...
                                .map((streak) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-right">
                                            <p className="text-sm font-medium text-yellow-800">{streak.riskLevel} horas restantes</p>
                                            <Button
                                                size="sm"
                                                variant="outline"
                                                onClick={() => handleStreakAction(streak.studentId, "extend")}
                                            >
                                                Extender
                                            </Button>
                                        </div>
